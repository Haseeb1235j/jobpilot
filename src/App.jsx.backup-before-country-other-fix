...
function buildCleanResumeHtml(profile) {
  const hasValue = (value) => value && String(value).trim().length > 0
  ...
  const bulletList = (items) => {
    ...
    return `<ul>${cleanItems.map((item) => `<li>${clean(item)}</li>`).join("")}</ul>`
  }

  const contactParts = []
  if (hasValue(profile?.phone)) contactParts.push(`Phone: ${clean(profile.phone)}`)
  if (hasValue(profile?.email)) contactParts.push(`Email: ${clean(profile.email)}`)

  const linkParts = []
  if (hasValue(profile?.linkedin)) linkParts.push(`LinkedIn: ${clean(profile.linkedin)}`)
  if (hasValue(profile?.github)) linkParts.push(`GitHub: ${clean(profile.github)}`)
  if (hasValue(profile?.portfolio)) linkParts.push(`Portfolio: ${clean(profile.portfolio)}`)
  ...
  const projects = Array.isArray(profile?.projects)
    ? profile.projects.filter(
        (project) => hasValue(project.name) || hasValue(project.description)
      )
    : []

  const educations = Array.isArray(profile?.educations)
    ? profile.educations.filter(
        (edu) => hasValue(edu.degree) || hasValue(edu.college)
      )
    : []

  const certifications = Array.isArray(profile?.certifications)
    ? profile.certifications.filter((cert) => hasValue(cert.name))
    : []

  const achievements = Array.isArray(profile?.achievements)
    ? profile.achievements.filter(
        (ach) => hasValue(ach.title) || hasValue(ach.description)
      )
    : []

  const languages = Array.isArray(profile?.languages)
    ? profile.languages.filter((lang) => hasValue(lang.name))
    : []
    ...
    .map(
      (project) => `
        <div class="resume-item">
          ${hasValue(project.name) ? `<div class="resume-item-title">${clean(project.name)}</div>` : ""}
          ${hasValue(project.tech) ? `<div class="resume-item-sub">Tech Stack: ${clean(project.tech)}</div>` : ""}
          ${
            hasValue(project.description) || hasValue(project.link)
              ? `<ul>
                  ${hasValue(project.description) ? `<li>${clean(project.description)}</li>` : ""}
                  ${hasValue(project.link) ? `<li>Link: ${clean(project.link)}</li>` : ""}
                </ul>`
              : ""
          }
        </div>
  ...
  const educationHtml = educations
    .map(
      (edu) => `
        <div class="resume-item">
          ${hasValue(edu.degree) ? `<div class="resume-item-title">${clean(edu.degree)}</div>` : ""}
          ${hasValue(edu.college) ? `<div class="resume-item-sub">${clean(edu.college)}</div>` : ""}
          ${
            hasValue(edu.branch) || hasValue(edu.year) || hasValue(edu.score)
              ? `<div class="resume-item-meta">
                  ${hasValue(edu.branch) ? `Course/Branch: ${clean(edu.branch)}` : ""}
                  ${hasValue(edu.branch) && (hasValue(edu.year) || hasValue(edu.score)) ? " | " : ""}
                  ${hasValue(edu.year) ? `Year: ${clean(edu.year)}` : ""}
                  ${hasValue(edu.year) && hasValue(edu.score) ? " | " : ""}
                  ${hasValue(edu.score) ? `Score: ${clean(edu.score)}` : ""}
                </div>`
              : ""
          }
        </div>
        ...
        ${certifications
          .map(
            (cert) =>
              `<li>
                ${clean(cert.name)}
                ${hasValue(cert.issuer) ? ` - ${clean(cert.issuer)}` : ""}
                ${hasValue(cert.year) ? ` (${clean(cert.year)})` : ""}
                ${hasValue(cert.link) ? ` - ${clean(cert.link)}` : ""}
              </li>`
          )
          .join("")}
      </ul>`
    ...
    ? `<ul>
        ${achievements
          .map(
            (ach) =>
              `<li>
                ${hasValue(ach.title) ? `<strong>${clean(ach.title)}</strong>` : ""}
                ${hasValue(ach.title) && hasValue(ach.description) ? " - " : ""}
                ${hasValue(ach.description) ? clean(ach.description) : ""}
              </li>`
          )
          .join("")}
      </ul>`
        ...
        ${languages
          .map(
            (lang) =>
              `<li>
                ${clean(lang.name)}
                ${hasValue(lang.level) ? ` - ${clean(lang.level)}` : ""}
              </li>`
          )
          .join("")}
      </ul>`
  ...
  return `
    <div class="resume-header">
      ${hasValue(profile?.name) ? `<h1>${clean(profile.name)}</h1>` : ""}
      ${contactParts.length ? `<p>${contactParts.join(" | ")}</p>` : ""}
      ${linkParts.length ? `<p>${linkParts.join(" | ")}</p>` : ""}
      ${hasValue(profile?.location) ? `<p>Location: ${clean(profile.location)}</p>` : ""}
    </div>
    ...
    ${
      tools.length || strengths.length
        ? `<section>
            <h2>Tools & Other Skills</h2>
            ${bulletList([...tools, ...strengths])}
          </section>`
        : ""
    }
      ...
      hasValue(profile?.name)
        ? `<section>
            ...
            <div class="declaration-grid">
              <div>
                ${hasValue(profile?.location) ? `<p><strong>Place:</strong> ${clean(profile.location)}</p>` : ""}
                <p><strong>Date:</strong> ${new Date().toLocaleDateString()}</p>
              </div>
              <div>
                <p><strong>Signature:</strong></p>
                <p>${clean(profile.name)}</p>
              </div>
            </div>
          </section>`
        : ""
...
function getProfessionalDocumentHtml(
  ...
  const cleanName = escapeHtml(profile?.name || "Candidate")
  const cleanRole = escapeHtml(profile?.role || "Career Document")
  const cleanEmail = escapeHtml(profile?.email || "")
  const cleanPhone = escapeHtml(profile?.phone || "")
  const cleanLocation = escapeHtml(profile?.location || "")

  const documentLabel = isResume
    ? "Professional Resume"
    : isLetter
    ? "Application Letter"
    : "JobPilot AI Document"

  const contentBody = isResume
    ? resumeBody
    : `
      <section>
        <h2>${escapeHtml(documentLabel)}</h2>
        <div class="ai-document-body">
          <p>${normalBody}</p>
        </div>
      </section>
    `
...
<head>
  ...
  <style>
    ...
    .side-role{font-size:13px;color:#475569;text-transform:uppercase;margin-bottom:20px;letter-spacing:.8px}
    ...
    .side-text{font-size:12px;line-height:1.55;word-break:break-word;color:#1f2937;margin-bottom:5px}
    .resume-header{display:none}
    section{margin-top:0;margin-bottom:16px}
    ...
    h3{font-size:14px;margin:12px 0 6px;color:#111827}
    p,li{font-size:12.8px;line-height:1.6;margin:0 0 7px}
    ul{margin:0;padding-left:18px}
    li{margin-bottom:4px}
    .resume-item{margin-bottom:11px}
...
<body>
  <div class="page">
    <aside class="sidebar">
      <div class="side-name">${cleanName}</div>
      <div class="side-role">${cleanRole}</div>

      <div class="side-block">
        <div class="side-title">Document</div>
        <div class="side-text">${escapeHtml(documentLabel)}</div>
        <div class="side-text">${new Date().toLocaleDateString()}</div>
      </div>

      <div class="side-block">
        <div class="side-title">Contact</div>
        ${cleanEmail ? `<div class="side-text">${cleanEmail}</div>` : ""}
        ${cleanPhone ? `<div class="side-text">${cleanPhone}</div>` : ""}
        ${cleanLocation ? `<div class="side-text">${cleanLocation}</div>` : ""}
      </div>
      ...
      ${
        profile?.technicalSkills
          ? `<div class="side-block">
              ...
              <div class="side-text">${escapeHtml(profile.technicalSkills)}</div>
            </div>`
          : ""
      }
    </aside>
...
<head>
  ...
  <style>
    ...
    .resume-header,.doc-header{text-align:center;border-bottom:3px solid #222;padding-bottom:14px;margin-bottom:18px}
    .resume-header h1,.doc-header h1{font-family:Arial,Helvetica,sans-serif;margin:0 0 8px;font-size:29px;letter-spacing:4px;text-transform:uppercase;color:#111}
    .resume-header p,.doc-header p{font-family:Arial,Helvetica,sans-serif;font-size:12px;color:#4b5563;margin:3px 0;line-height:1.4}
    ...
    p,li{font-size:12.8px;line-height:1.62;margin:0 0 7px}
    ul{margin:0;padding-left:18px}
    li{margin-bottom:4px}
      ...
      isResume
        ...
        : `
          <div class="doc-header">
            ...
            ${cleanLocation ? `<p>${cleanLocation}</p>` : ""}
          </div>
          ${contentBody}
        `
    }
...
<head>
  ...
  <style>
    ...
    .resume-header,.doc-header{text-align:center;padding-bottom:14px;border-bottom:3px solid #1f4e79;margin-bottom:18px}
    .resume-header h1,.doc-header h1{margin:0 0 8px;font-size:28px;letter-spacing:1px;color:#1f4e79;text-transform:uppercase;font-weight:800}
    .resume-header p,.doc-header p{margin:3px 0;font-size:12.5px;line-height:1.4;color:#374151}
    ...
    h3{font-size:14px;margin:12px 0 6px;color:#111827}
    p{font-size:12.8px;line-height:1.62;margin:0 0 8px}
    ul{margin:0;padding-left:18px}
    li{font-size:12.8px;line-height:1.55;margin-bottom:4px}
    .resume-item{margin-bottom:10px}
    .resume-item-title{font-size:13px;font-weight:700;color:#111827;margin-bottom:2px}
  ...
  return (
    <div className="mb-4 bg-black/20 border border-white/10 rounded-2xl p-4">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 mb-4">
        <div>
          <h3 className="text-lg font-bold">Choose Export Template</h3>
          <p className="text-sm text-gray-400">
            Templates apply to resume, cover letter, interview prep, skill roadmap,
            and all exported AI documents.
          </p>
        </div>
...
<body>
  ...
  const detectSmartDocType = (cleanMessage, defaultType) => {
    ...
    if (
      ...
      lowerMessage.includes("add that in resume") ||
      lowerMessage.includes("put this in resume") ||
      lowerMessage.includes("update my resume") ||
      lowerMessage.includes("improve my resume") ||
      lowerMessage.includes("make my resume")
    ) {
      return "resume"
    }
  ...
  const askAgent = async (messageText, documentType = "chat") => {
    ...
    const smartDocType = detectSmartDocType(cleanMessage, documentType)
          ...
          <div className="bg-white/5 border border-white/10 rounded-3xl p-5 h-fit">
            ...
            <div className="space-y-3">
              <ActionButton
                ...
                onClick={() =>
                  quickAskAgent(
                    "Generate a complete professional ATS-friendly resume using all available candidate profile details. Do not show missing fields. Do not write 'Not provided'. If a detail is empty, skip it. Include only clean filled sections: Professional Summary, Technical Skills, Projects, Education, Certifications, Strengths, and Declaration if suitable. Make it job-ready and truthful.",
                    "resume"
                  )
                }
              >
